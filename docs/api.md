...
## 39.5 智能报告任务 (Report Tasks)

用户自定义定时报告：cron 调度（6 段含秒）+ Scope 标签圈定 + AI 解读，结果以 Card 2.0 图表卡片推送飞书群。

| 方法 | 路由 | 访问级别 | 说明 |
|------|------|----------|------|
| GET | `/report-tasks` | 管理权限 | 任务列表。筛选：`?enabled=true` |
| GET | `/report-tasks/:id` | 管理权限 | 按 ID 获取 |
| POST | `/report-tasks` | 管理权限 | 创建任务（cron 6 段校验，非法返回 10001） |
| PUT | `/report-tasks/:id` | 管理权限 | 更新任务（id/created_by 服务端保护，热更新调度） |
| DELETE | `/report-tasks/:id` | 管理权限 | 删除任务并移出调度器 |
| POST | `/report-tasks/:id/run` | 操作权限 | 立即执行（异步） |
| GET | `/report-runs` | 管理权限 | 执行记录（分页）。筛选：`?task_id=` |
| GET | `/report-runs/:id` | 管理权限 | 执行详情（含 report_markdown / findings_json） |

**任务字段：** `name`、`description`、`cron_expr`（6 段含秒，如 `0 0 9 * * *`）、`report_type`（daily \| weekly \| custom）、`scope`（JSON：`{"match_labels":{...},"time_range_hours":24}`，match_labels 支持 `!=`/`=~`/`!~` 前缀）、`prompt_template`、`allowed_tools`（JSON 数组，空 = 只读默认集）、`output_channels`（JSON：`[{"type":"lark_bot","bot_id":"oc_xxx"},{"type":"webhook","url":"..."}]`）、`enabled`。

**统计边界：** 报告中的数字（总量/环比/severity 分布/Top5/趋势/MTTA/MTTR）由平台从 `alert_events` 直接计算并注入提示词，LLM 仅做解读。

### 飞书卡片回调

| 方法 | 路由 | 访问级别 | 说明 |
|------|------|----------|------|
| POST | `/lark/card-action` | 公开（签名/Token 校验 + 时间戳反重放） | 卡片按钮回调。操作需操作者已绑定平台账号且角色具备 `events.ack`/`events.assign` 权限；v2 卡片返回 toast，v1 返回替换卡片 |
| GET | `/api/v1/lark/bot/status` | 已认证 | Bot 状态（含 `connection_mode` 与 WS 长连接 `event_source_status`） |
