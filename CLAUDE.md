...
> **v4.72.0** | Go 1.25 + Gin + Vue 3 + MySQL 8 + Redis 7
...
internal/
  model/ (62) handler/ (77) service/ (98) repository/ (59)
...
`SREAGENT_DATABASE_PASSWORD` / `SREAGENT_REDIS_PASSWORD` / `SREAGENT_JWT_SECRET` / `SREAGENT_SECRET_KEY` / `SREAGENT_ADMIN_PASSWORD` / `SREAGENT_ADMIN_PASSWORD_FORCE` / `SREAGENT_WEBHOOK_SECRET` / `SREAGENT_MAX_CONCURRENT_SENDS` / `SREAGENT_JWT_REFRESH_GRACE`
