...
> **v4.63.0** | Go 1.25 + Gin + Vue 3 + MySQL 8 + Redis 7
...
`SREAGENT_DATABASE_PASSWORD` / `SREAGENT_REDIS_PASSWORD` / `SREAGENT_JWT_SECRET` / `SREAGENT_SECRET_KEY` / `SREAGENT_ADMIN_PASSWORD` / `SREAGENT_ADMIN_PASSWORD_FORCE` / `SREAGENT_WEBHOOK_SECRET` / `SREAGENT_MAX_CONCURRENT_SENDS`
