...
test.describe('T3 - 通知管道', () => {
  ...
  // T3-1: 通知策略列表
  test('T3-1 通知策略列表', async ({ authenticatedPage: page }) => {
    await expect(page.locator('body')).toBeVisible()
  })

  // T3-2: 新建通知策略
  test('T3-2 新建按钮', async ({ authenticatedPage: page }) => {
...
test.describe('T3 - 通知渠道', () => {
  ...
  // T3-10: 渠道列表
  test('T3-10 渠道列表', async ({ authenticatedPage: page }) => {
    await expect(page.locator('body')).toBeVisible()
  })
})
...
test.describe('T3 - 消息模板', () => {
  ...
  // T3-20: 模板列表
  test('T3-20 模板列表', async ({ authenticatedPage: page }) => {
    await expect(page.locator('body')).toBeVisible()
  })
})
