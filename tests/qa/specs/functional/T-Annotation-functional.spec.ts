...
async function createAnnotation(page: any, overrides: Record<string, unknown> = {}) {
  ...
  const now = new Date()
  const payload = {
    text: `Functional test annotation ${tag}`,
    dashboard_id: 1,
...
test('AN-1 标注 CRUD', async ({ authPage: page }) => {
  ...
  try {
    await test.step('创建标注', async () => {
      ...
      expect(annotation.text).toContain('Functional test annotation')
      await page.screenshot({ path: 'test-results/AN-1-01-创建成功.png', fullPage: false })
    })

    await test.step('GET 验证标注已保存', async () => {
      const res = await API.get(page, `${API_BASE}/annotations?dashboard_id=1`)
      expect(res.code).toBe(0)
      const list = res.data?.list || res.data || []
      const found = Array.isArray(list) && list.find((a: any) => a.id === annotationId)
      expect(found).toBeTruthy()
      expect(found.text).toContain('Functional test annotation')
    ...
    await test.step('验证更新生效', async () => {
      ...
      const found = Array.isArray(list) && list.find((a: any) => a.id === annotationId)
      expect(found).toBeTruthy()
      expect(found.text).toBe('Updated by functional test')
  ...
  try {
    ...
    await test.step('验证批量创建的标注均存在', async () => {
      ...
      for (const id of annotationIds) {
        const found = Array.isArray(list) && list.find((a: any) => a.id === id)
        expect(found).toBeTruthy()
      }
...
test('AN-3 标注时间范围查询', async ({ authPage: page }) => {
  ...
  try {
    ...
    await test.step('按时间范围查询', async () => {
      ...
      const res = await API.get(page, `${API_BASE}/annotations?dashboard_id=1&from=${threeHoursAgo}&to=${now}`)
      expect(res.code).toBe(0)
