...
/** Helper: delete an inspection task by ID, ignoring errors (for cleanup) */
...
test('IN-3 巡检 cron校验', async ({ authPage: page }) => {
  await test.step('有效 cron 表达式校验', async () => {
    const res = await API.post(page, `${API_BASE}/inspection/validate-cron`, {
      cron_expr: '0 0 0 * * *',
    })
    expect(res.code).toBe(0)
...
test('IN-4 巡检运行记录', async ({ authPage: page }) => {
  await test.step('获取巡检运行记录列表', async () => {
    const res = await API.get(page, `${API_BASE}/inspection/runs?page=1&page_size=20`)
    expect(res.code).toBe(0)
    expect(res.data).toBeDefined()
  ...
  await test.step('验证运行记录结构', async () => {
    const res = await API.get(page, `${API_BASE}/inspection/runs?page=1&page_size=5`)
    expect(res.code).toBe(0)
