...
test('BD-2 内置仪表盘 apply导入', async ({ authPage: page }) => {
  ...
  if (dashboardId) {
    await test.step('Apply 导入内置仪表盘', async () => {
      const res = await API.post(page, `${API_BASE}/builtin-dashboards/${dashboardId}/apply`)
      // Apply may succeed or return conflict if already imported
      expect(res).toBeDefined()
      expect(res.code).toBeDefined()
      await page.screenshot({ path: 'test-results/BD-2-02-Apply结果.png', fullPage: false })
    })

    await test.step('验证 Apply 结果', async () => {
