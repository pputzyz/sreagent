...
test.describe('T2 - 告警事件', () => {
  ...
  // T2-1: 列表加载
  test('T2-1 事件列表加载', async ({ authenticatedPage: page }) => {
    await expect(page.locator('[class*="event"], .event-card, .event-row')).toHaveCount({ minimum: 0 })
  })

  // T2-2: 筛选器
  test('T2-2 状态筛选', async ({ authenticatedPage: page }) => {
  ...
  // T2-3: 时间范围筛选
  test('T2-3 时间范围', async ({ authenticatedPage: page }) => {
  ...
  // T2-4: 事件详情
  test('T2-4 点击事件查看详情', async ({ authenticatedPage: page }) => {
  ...
  // T2-5: 认领操作
  test('T2-5 认领按钮', async ({ authenticatedPage: page }) => {
