...
var translations = map[string]map[string]string{
	ZhCN: {
		...
		// NOTE: service-layer fmt.Errorf strings are intentionally NOT translated here.
		// Those errors are Chinese at the source and are surfaced to users directly
		// (run.error_msg, Lark tool replies) without passing through handler.Error /
		// LocalizeMessage, so catalog entries for them would be dead code. Per-recipient
		// localization of outbound text is a separate effort (see docs/i18n.md / ADR-16).
	},
}
